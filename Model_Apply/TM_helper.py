...
def coordinate_calculation(pdb,path):
    class eigen:
        def __init__(self):
                self.x=0
    ...
    tm = np.load(path + "Intermediate_Data/TM_matrix_2_pdbs-noTER_symmetric.npy")
    tm = tm + np.identity(len(tm))*5.6
    tm_test = TM_1200(pdb, path + 'Final_Data/pdbs/')

    model=eigen()
    tm_test_new = model.center(np.array([tm_test]), tm)
    fold_basis=np.loadtxt(path + "Final_Data/folds_basis")

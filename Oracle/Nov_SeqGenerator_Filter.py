...
if O_DeepSF_flag:
    ...
    O_Seq_Stat_file = open(generated_file_index + '_Origin_All_Stat','w')
...
with tf.Session() as session:
    ...
    all_num = 0
    suc_num = '-'
    seq_file = open(generated_file_index + '_GenSeq','w')
    seq_file.close()
    time_file = open(generated_file_index + '_time_stat','w')
    time_file.write('valid_num\tsuc_num\ttime\n')
    time_file.close()

    if M_DeepSF_flag or O_DeepSF_flag:
        Stat_file = open(generated_file_index + '_SeqStatus','w')
        Stat_file.write('M_DeepSF\tO_DeepSF\n')
        Stat_file.close()
        Suc_file = open(generated_file_index + '_SucSeq','w')
        Suc_file.close()
    ...
    while((all_num < All_Num) or Suc_Flag):
        ...
        for sa in samples:
            ...
            if ((len(samp) >= MIN_LEN) and (len(samp) <= MAX_LEN)) and ((not ('!' in samp)) and sam[0] != '!'):
                ...
                with open(generated_file_index + '_GenSeq','a') as seq_file: 
                    seq_file.write(samp + '\n')
        ...
        ############## Activate the modified DeepSF ##########
            ...
            if O_DeepSF_flag:
                ...
                for p in success_index:  
                    ...
                    O_Seq_Stat_file = open(generated_file_index + '_Origin_All_Stat','a')
                    O_Seq_Stat_file.write(seq + '\n')
                    O_Seq_Stat_file.write(str(predict_labels) + '\n')
                     
                    for f_p in predict_labels:
                        if f_p in folds_dict.keys():
                            if np.linalg.norm(novel_array - np.array(folds_dict[f_p])) <= THRESHOLD:
        ...
        ### Record the sequence status and the successful sequences ###
            ...
            Stat_file = open(generated_file_index + '_SeqStatus','a')
            for p in range(V_SIZE):
                Stat_file.write(seq_status[p][0] + '\t')
                Stat_file.write(seq_status[p][1] + '\n')
            Stat_file.close()

            Suc_file = open(generated_file_index + '_SucSeq','a')
            for s in samples_f:
                Suc_file.write(s + '\n')
            Suc_file.close()
        ...
        with open(generated_file_index + '_time_stat','a') as time_file:

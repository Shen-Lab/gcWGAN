...
if O_DeepSF_flag:
    ...
    O_Seq_Stat_file = open(generated_file_index + '_Origin_All_Stat' + '_with_TMscore','w')
...
with tf.Session() as session:
    ...
    all_num = 0
    suc_num = '-'
    seq_file = open(generated_file_index + '_GenSeq' + '_with_TMscore','w')
    seq_file.close()
    time_file = open(generated_file_index + '_time_stat' + '_with_TMscore','w')
    time_file.write('valid_num\tsuc_num\ttime\n')
    time_file.close()

    if M_DeepSF_flag or O_DeepSF_flag:
        Stat_file = open(generated_file_index + '_SeqStatus' + '_with_TMscore','w')
        Stat_file.write('M_DeepSF\tO_DeepSF\n')
        Stat_file.close()
        Suc_file = open(generated_file_index + '_SucSeq' + '_with_TMscore','w')
        Suc_file.close()
    ...
    while((all_num < All_Num) or Suc_Flag):
        ...
        for sa in samples:
            ...
            if ((len(samp) >= MIN_LEN) and (len(samp) <= MAX_LEN)) and ((not ('!' in samp)) and sam[0] != '!'):
                ...
                with open(generated_file_index + '_GenSeq' + '_with_TMscore','a') as seq_file: 
                    seq_file.write(samp + '\n')
        ...
        ############## Activate the modified DeepSF ##########
            ...
            if O_DeepSF_flag:
                ...
                for p in success_index:  
                    ...
                    O_Seq_Stat_file = open(generated_file_index + '_Origin_All_Stat' + '_with_TMscore','a')
                    O_Seq_Stat_file.write(seq + '\n')
                    O_Seq_Stat_file.write(str(predict_labels) + '\n')
                     
                    if ('a.30' in predict_labels) or ('a.60' in predict_labels) or ('a.53' in predict_labels):
        ...
        ### Record the sequence status and the successful sequences ###
            ...
            Stat_file = open(generated_file_index + '_SeqStatus' + '_with_TMscore','a')
            for p in range(V_SIZE):
                Stat_file.write(seq_status[p][0] + '\t')
                Stat_file.write(seq_status[p][1] + '\n')
            Stat_file.close()

            Suc_file = open(generated_file_index + '_SucSeq' + '_with_TMscore','a')
            for s in samples_f:
                Suc_file.write(s + '\n')
            Suc_file.close()
        ...
        with open(generated_file_index + '_time_stat' + '_with_TMscore','a') as time_file:

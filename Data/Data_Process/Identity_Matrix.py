...
inter_dic = Assessment.Train_dic(DATA_DIR + 'fold_train', DATA_DIR + 'seq_train')
...
for fold in fold_list:
    ...
    #size = inter_dic[fold][0]
    size = len(inter_dic[fold])
    print fold,size
    iden_matrix = np.zeros([size,size])
    #nature_seq = [i.upper() for i in inter_dic[fold][1:]]
    nature_seq = [i.upper() for i in inter_dic[fold]]
    ...
    np.save(output_path + fold_name,iden_matrix)

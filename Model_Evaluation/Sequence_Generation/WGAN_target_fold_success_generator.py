##################################################################################
# Generate the sequences whose top ten predictions contain at least one target 
# fold based on cWGAN or gcWGAN.
##################################################################################
...
target_name = '_'.join(['_'.join(t.split('.')) for t in TARGET])
...
if 'nov' in FOLD.lower():
    ...
    fold_vector = folds_dict[FOLD]

print 'Model:',KIND
print 'FOLD:',FOLD
print 'Fold coordinate:',fold_vector
print 'Target folds:',TARGET
...
s_file = open('Pipeline_Sample/' + KIND + '_Fasta_' + str(GEN_NUM) + '_success_' + fold_name + '_target_' + target_name,'w')
s_file.close()
    ...
    while(num < GEN_NUM):
        ...
        for p in range(V_SIZE):
            ...
            for f_t in TARGET:
                if f_t in f_pre:
                    num += 1
                    s_file = open('Pipeline_Sample/' + KIND + '_Fasta_' + str(GEN_NUM) + '_success_' + fold_name + '_target_' + target_name,'a')
                    sequ = ''.join(test_se[p])

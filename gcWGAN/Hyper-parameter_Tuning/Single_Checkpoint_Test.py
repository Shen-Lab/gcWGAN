##################################################################################################
# gcWGAN Nonsense Ratio, Sequence Identity, Padding Ratio and Stability Calculation on a single 
# check point.
# Requirement: Have finished gcWGAN Training
# Input: check point index, epoch index, the data set ("train", "vali" or "test") and the required 
#        valid sequence number.
# Output: print out the values gotten
##################################################################################################
...
epoch_index = int(sys.argv[2])
fold_set = sys.argv[3]
GENERATE_SIZE = int(sys.argv[4]) #required valid sequence number
...
with tf.Session() as session:
    ...
    for c in Check_selected:
        ...
        nonsen_ratio = np.mean(N_R)
        ident = np.mean(Ide)
        pad_ratio = np.mean(P_R)
        mean_stability = np.mean(stab)

        print ''
        print 'Check point:',test_index
        print 'Epoch:',epoch_index
        print 'Data set:',fold_set
        print 'Valid sequence amount:',GENERATE_SIZE
        print ''
        print 'Nonsense sequence ratio:',nonsen_ratio
        print 'Sequence Identity:',ident
        print 'Padding Ratio:',pad_ratio
        print 'Sequence stability:',mean_stability

...
fold_len = 20 #MK add
name = '_' + str(Learning_rate) + '_' + str(CRITIC_ITERS) + '_' + str(noise_len) + '_' + str(LAMBDA_g)+'_without_semi'
...
check_dic = {}
f_list = os.listdir('../Checkpoints/cWGAN/model_0.0001_5_64/')
for f in f_list:
    f = f.split('.')[0].split('_')
    if f[0] == 'model':
        check_dic[f[1]] = f[2]
...
with tf.Session() as session:
    ...
    saver.restore(session,"../Checkpoints/cWGAN/model_0.0001_5_64/model_"+str(c_restore)+"_" + check_dic[str(c_restore)] + ".ckpt")
    DLS2F_ResCNN.load_weights(model_weight)
    DLS2F_ResCNN.trainable = False

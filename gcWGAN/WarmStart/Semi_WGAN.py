...
check_path = '../../Checkpoints/WarmStart/Labeled_Training/'
sample_path = 'WarmStart_Samples/Labeled_Training/'
...
#os.system('mkdir Check_Points'+NAME)
...
check_dic = {}
f_list = os.listdir('../../Checkpoints/WarmStart/Unlabeled_Training/')
for f in f_list:
    f = f.split('.')[0].split('_')
    if f[0] == 'model':
        check_dic[f[1]] = f[2]
...
with tf.Session() as session:
    ...
    saver.restore(session,"../../Checkpoints/WarmStart/Unlabeled_Training/model_"+str(c_restore)+"_" + check_dic[str(c_restore)] + ".ckpt")
...
plt.savefig('Images/Critic_Semisupervise.png')
plt.figure(2) #SZ add
plt.plot(G_cost) #SZ add
plt.title("Generator Loss Function")
plt.xlabel("iteration number")
plt.ylabel("generator loss")
plt.savefig('Images/Generator_Semisupervise.png')
plt.show() #SZ add
"""
